...
def _validate_visualization(viz: dict[str, Any], columns: list[str]) -> bool:
    """Validate a single visualization spec before sending to the frontend.

    Ensures the spec uses data.name='table' (no inline data), references only
    existing columns, uses allowed mark types, and isn't excessively large.
    """
    try:
        spec = viz.get("spec", {})

        # Must reference named dataset, not inline data
        data = spec.get("data", {})
        if data.get("name") != "table":
            logger.warning("Visualization rejected: data.name must be 'table'")
            return False
        if "values" in data:
            logger.warning("Visualization rejected: inline data.values not allowed")
            return False

        # Check mark type allowlist
        mark = spec.get("mark")
        mark_type = mark.get("type") if isinstance(mark, dict) else mark
        allowed_marks = {"bar", "line", "area", "point", "arc", "rect"}
        if mark_type not in allowed_marks:
            logger.warning("Visualization rejected: mark type '%s' not allowed", mark_type)
            return False

        # Validate encoding field references exist in columns
        encoding = spec.get("encoding", {})
        columns_lower = {c.lower() for c in columns}
        for _channel, channel_def in encoding.items():
            if isinstance(channel_def, dict):
                field = channel_def.get("field")
                if field and field.lower() not in columns_lower:
                    logger.warning("Visualization rejected: field '%s' not in columns", field)
                    return False

        # Cap spec size
        if len(json.dumps(spec)) > 50_000:
            logger.warning("Visualization rejected: spec too large")
            return False

        return True
    except Exception:
        logger.warning("Visualization validation failed", exc_info=True)
        return False


def _generate_visualizations(user_query: str, result: dict[str, Any]) -> list[dict[str, Any]] | None:
    """Generate chart visualizations for query results.

    Calls the LLM to produce Vega-Lite specs, validates them, and returns
    up to 2 valid charts. Returns None on any error or when data is not chartable.
    """
    try:
        llm = get_llm_service()
        ...
        # Skip for empty or trivial results
        if row_count == 0 or (row_count == 1 and len(columns) == 1):
            return None

        visualizations = llm.generate_visualizations(
            user_query=user_query,
            columns=columns,
            rows=rows,
            row_count=row_count,
        ...
        # Validate and cap at 2 charts
        valid = [v for v in visualizations[:2] if _validate_visualization(v, columns)]
...
def format_response(state: AgentState) -> dict[str, Any]:
    ...
    Includes a natural language summary and chart visualizations generated by the LLM.
    ...
    with langfuse_span("format_response", input={"row_count": result.get("row_count", 0)}) as span:
        ...
        visualizations = _generate_visualizations(state["user_query"], result)
