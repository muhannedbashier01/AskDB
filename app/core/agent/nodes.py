...
from app.core.domain.policies import POLICIES_TABLES
...
def summarize_response(state: AgentState) -> dict[str, Any]:
    """Generate a natural language summary of the SQL execution result.

    Dedicated node that calls the LLM to produce a concise summary.
    Falls back gracefully — the response is still returned without a summary
    if generation fails. Keeping this as a separate node ensures that
    ``format_response`` is a pure data-assembly step with no LLM calls.

    Args:
        state: Current agent state (execution_result must be populated).

    Returns:
        State update with ``nl_summary`` (str | None).
    """
    from app.core.prompts.sql_agent import RESPONSE_FORMAT_PROMPT

    result = state["execution_result"]
    if not result:
        return {"nl_summary": None}
    ...
    try:
        ...
        logger.info("NL summary generated", has_summary=summary is not None)
        return {"nl_summary": summary}
    except Exception:
        logger.warning("Failed to generate NL summary, continuing without it", exc_info=True)
        return {"nl_summary": None}


def format_response(state: AgentState) -> dict[str, Any]:
    """Assemble the final success response from agent state.

    Pure data-assembly step — no LLM calls. Summary is sourced from
    ``nl_summary`` which is populated by the preceding ``summarize_response``
    node.
